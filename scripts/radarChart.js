...
function RadarChart(id, data, options) {
	...
	var maxValue = Math.max(cfg.maxValue, d3version3.max(data, function(i){return d3version3.max(i.map(function(o){return o.value;}))}));

	var allAxis = (data[0].map(function(i, j){return i.axis})),	//Names of each axis
		total = allAxis.length,					//The number of different axes
		radius = Math.min(cfg.w/2, cfg.h/2), 	//Radius of the outermost circle
		Format = d3version3.format('%'),			 	//Percentage formatting
	...
	var radarLine = d3version3.svg.line.radial()
	...
	blobCircleWrapper.selectAll(".radarInvisibleCircle")
		...
		.on("mouseover", function(d,i) {
			newX =  parseFloat(d3version3.select(this).attr('cx')) - 10;
			newY =  parseFloat(d3version3.select(this).attr('cy')) - 10;
	...
	function wrap(text, width) {
	  text.each(function() {
		var text = d3version3.select(this),
